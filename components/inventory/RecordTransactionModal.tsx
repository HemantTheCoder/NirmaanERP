...
import { createInventoryTransaction, type InventoryTransactionType } from "@/lib/queries/inventory";
...
export function RecordTransactionModal({ isOpen, onClose, userId, items, onCreated }: RecordTransactionModalProps) {
  const supabase = createClient();
  const [itemId, setItemId] = useState("");
  const [type, setType] = useState<InventoryTransactionType>("receipt");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: createError } = await createInventoryTransaction(supabase, {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-card border border-border rounded-2xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <h2 className="text-base font-bold text-foreground">Record Stock Transaction</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div>
            ...
            >
              ...
              {items.map((i) => (
                <option key={i.id} value={i.id}>{i.name} ({i.quantity_on_hand} {i.unit} in stock)</option>
              ))}
            </select>
          </div>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground">Reference (PO number, task, etc.)</label>
            <input
              type="text"
              value={reference}
